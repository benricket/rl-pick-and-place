...
from kinematics import KinovaLiteDH
...
class ArmEnv(gym.Env):
    ...
    def _compute_reward(self):
        ...
        progress_reward = 100.0 * (self.last_ee_to_block_dist - dist_norm) * 1
    ...
    def step(self, action):
        ...
        max_pos_change = 0.2 # meters
        max_ang_change = 0.5 # rad of end effector orientation
        max_joint_change = 0.4 # rad of joint
        max_change = np.array(3 * [max_pos_change] + 3 * [max_ang_change])

        # Apply action
        # Get desired pose given change in XYZ RPY
        joint_ctrl = action[0:6]
        joint_ctrl *= max_change 

        # Get current pose as start for numerical IK
        target = EndEffector()
        ee_name = "pinch_site"
        ee_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_SITE, ee_name)
        pos_now = self.data.site_xpos[ee_id].copy()
        rot_now = rotm_to_euler(self.ee_rotm)
        current_pose = np.hstack([pos_now,rot_now])
        #print(f"current pose shape {current_pose.shape}, pose {current_pose}")

        target.x = pos_now[0] + joint_ctrl[0]
        target.y = pos_now[1] + joint_ctrl[1]
        target.z = pos_now[2] + joint_ctrl[2]
        target.rotz = rot_now[0] + joint_ctrl[3]
        target.roty = rot_now[1] + joint_ctrl[4]
        target.rotx = rot_now[2] + joint_ctrl[5]

        curr_joint_vals = get_joints(self.model,self.data)
        joints = self.robot.calc_inverse_kinematics(target,q_guess=curr_joint_vals)
        joint_change = joints - curr_joint_vals
        joint_change = np.clip(joint_change,a_min=-1*max_joint_change,a_max=max_joint_change)
        ...
        #curr_joint_vals = get_joints(self.model,self.data)
        target_joint_vals = curr_joint_vals + joint_change
        self.joint_ctrl = joint_change
...
if __name__ == "__main__":
    ...
    #     clip_range=0.1,
    #     tensorboard_log=f"../logs/ppo_kinova_task_{time_str}/",
    #     #n_steps=2048,
    # )
    model = SAC(
        "MlpPolicy", 
        ...
        verbose=1,
        learning_rate=0.00004,
        tensorboard_log=f"../logs/sac_kinova_task_{time_str}/",
    ...
    log_name = "sac_task_curriculum"
    try:
        #model.learn(total_timesteps=2_000_000,callback=RewardPrintCallback(keys={}))
        env.env_method("set_random_level", 0.01)
        model.learn(total_timesteps=200_000,callback=RewardPrintCallback(keys=cb_keys),tb_log_name=log_name)
        ...
        model.save(f"kinova_task_{time_str}")
